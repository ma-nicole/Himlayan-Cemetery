...
const ServiceRequestManagementPage = () => {
  ...
  const handleOpenModal = (item) => {
    ...
    setFormData({
      status: isPaymentVerified ? 'completed' : item.status,
  ...
  const handleConfirmedDelete = async () => {
    try {
      await api.delete(`/service-requests/${archiveConfirmId}`);
      setArchiveConfirmId(null);
      loadRequests();
      loadStats();
    } catch (error) {
      setArchiveError('Failed to archive request. Please try again.');
    }
  };
      ...
      {archiveConfirmId !== null && (
        ...
        >
          ...
          >
            ...
            <h2 style={{ margin: '0 0 8px', fontSize: '22px', fontWeight: '700', color: '#111827' }}>Archive This Request?</h2>
            <p style={{ color: '#6b7280', fontSize: '14px', marginBottom: '6px' }}>Are you sure you want to archive this service request?</p>
